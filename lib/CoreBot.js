    ...
    function Conversation(task, message) {
        ...
        this.sayFirst = function(message) {
            ...
            if (typeof(message) == 'string') {
                console.log(this.source_message.channel);
                message = {
                    text: message,
        ...
        this.repeat = function(index) {
            if (this.sent.length) {
                var newIndex = index && this.sent[this.sent.length - 1 + index] ? this.sent[this.sent.length - 1 + index] : this.sent[this.sent.length - 1];
                this.messages.push(this.sent[newIndex]);
            } else {
        ...
        this.stop = function(status, message) {
            this.handler = null;
            this.messages = [];
